...
_RUNNER_ASSIGNED_NEXT = frozenset({
...
_ALLOWED = MappingProxyType({
    ...
    S.PROFILE_READY: frozenset({S.RUNNER_ASSIGNED, S.AVD_ASSIGNED, S.ERROR, S.DISABLED}),
    # AVD_ASSIGNED is retained only for compatibility with persisted P0 rows.
    S.AVD_ASSIGNED: _RUNNER_ASSIGNED_NEXT,
    S.RUNNER_ASSIGNED: _RUNNER_ASSIGNED_NEXT,
    ...
    S.THREADS_READY_FOR_HUMAN: frozenset({
        S.WAITING_HUMAN, S.THREADS_CREATED, S.NEEDS_VERIFICATION,
        S.RETRY_PENDING, S.ERROR,
    }),
    ...
    S.COOLDOWN: frozenset({S.RETRY_PENDING, S.RUNNER_ASSIGNED, S.AVD_ASSIGNED, S.ERROR}),
    S.RETRY_PENDING: frozenset({
        S.RUNNER_ASSIGNED, S.AVD_ASSIGNED, S.IG_READY_FOR_HUMAN,
        S.THREADS_READY_FOR_HUMAN, S.ACP_CONNECTING, S.ERROR, S.DISABLED,
    }),
