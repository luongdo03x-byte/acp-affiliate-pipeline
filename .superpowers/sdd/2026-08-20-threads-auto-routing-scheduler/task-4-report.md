...
- Preflight rejects empty inventory when `product.has_inventory=0` with `product_inventory_empty`.
- Preflight rejects stale product sync when `last_synced_at` or `last_seen_at` is missing/older than 48 hours with `product_sync_stale`.
